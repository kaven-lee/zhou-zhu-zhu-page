  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      const store = {
        ...
        load() {
          if (localStorage.getItem('schemaVersion') === '1') {
            this.state.config.size = JSON.parse(
              localStorage.getItem('configSize'),
            );
        ...
        persist() {
          ...
          localStorage.setItem(
            'configSize',
            JSON.stringify(this.state.config.size),
          );
          localStorage.setItem(
            'hideControls',
      ...
      function renderApp(state) {
        appNodes.pauseBtnSVG.setAttribute(
          'href',
          `#icon-${state.paused ? 'play' : 'pause'}`,
        );
        appNodes.shutterBtnSVG.setAttribute(
          'href',
          `#icon-shutter-${state.longExposure ? 'fast' : 'slow'}`,
        );
        ...
        appNodes.finaleModeLabel.style.opacity = state.config.autoLaunch
          ? 1
          : 0.32;
      ...
      const crysanthemumShell = (size = 1) => {
        ...
        const streamers =
          !pistil && color !== COLOR.White && Math.random() < 0.42;
        return {
          size: 300 + size * 100,
    ...
    <div>
      ...
      <div className={styles.container}>
        ...
        <div id="stage-container">
          <div className={styles.title}>请周总看一下烟花</div>
          <div id="canvas-container">
            <canvas id="trails-canvas"></canvas>
            <canvas id="main-canvas"></canvas>
          </div>
          <div id="controls">
            <div id="pause-btn" className={styles.btn}>
              <svg fill="white" width="24" height="24">
                <use href="#icon-pause"></use>
              </svg>
            </div>
            <div id="shutter-btn" className={styles.btn}>
              <svg fill="white" width="24" height="24">
                <use href="#icon-shutter-slow"></use>
              </svg>
            </div>
            <div id="settings-btn" className={styles.btn}>
              <svg fill="white" width="24" height="24">
                <use href="#icon-settings"></use>
              </svg>
            </div>
          </div>
          <div id="menu" className={styles.hide}>
            <div
              id="close-menu-btn"
              className={classnames(styles.btnBright, styles.btn)}
            >
              <svg fill="white" width="24" height="24">
            ...
            <form>
              ...
              >
                <label id="finale-mode-label">
                  <input id="finale-mode" type="checkbox" />
                  <span>Finale Mode</span>
                </label>
              </div>
              <div
                className={classnames(
              ...
              >
                <label id="hide-controls-label">
                  <input id="hide-controls" type="checkbox" />
